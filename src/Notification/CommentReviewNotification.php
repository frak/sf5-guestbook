...
class CommentReviewNotification extends Notification implements EmailNotificationInterface, ChatNotificationInterface
{
    ...
    public function asChatMessage(Recipient $recipient, string $transport = null): ?ChatMessage
    {
        ...
        $message->iconEmoji('tada')
            ->iconUrl('https://guestbook.wip/')
            ->userName('Guestbook')
            ->block((new SlackSectionBlock())->text($this->getSubject()))
            ->block(new SlackDividerBlock())
            ->block(
                (new SlackSectionBlock())->text(
                    "{$comment->getAuthor()} ({$comment->getEmail()}) says: {$comment->getText()}"
                )
            )
            ->block(
                (new SlackActionsBlock())->button('Accept', $this->reviewUrl, 'primary')->button(
                    'Reject',
